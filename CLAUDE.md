...
- **2026-05-14** — Полная система чатов: 1) **GroupChat + GroupChatMember** — новые Domain entities, EF Core конфигурации, AppDbContext обновлён; 2) **GroupChatDto/CreateGroupChatDto/ChatMessageDto/SendChatMessageDto** — расширены (GroupChatId, AttachmentFileId/Size); **UploadFileResponseDto** обновлён (FileId, FileName, Url, Size); 3) **FilesController** — upload (50МБ лимит, GUID-хранилище, безопасный путь) + download endpoints; 4) **GroupChatController** — CRUD (create/get/messages/add-member/remove-member/delete) + SignalR уведомление при добавлении участника; 5) **ChatController** обновлён — GetChats включает group summaries, SendMessage поддерживает GroupChatId; 6) **BoardHub** — новые методы JoinGroupChat/LeaveGroupChat; 7) **IChatApiService + ApiClient** — полная реализация файловых и групповых методов; 8) **ChatsViewModel** — полная страница чатов (список всех бесед, сообщения, отправка файлов, создание групп, SignalR real-time); 9) **ChatsView.xaml** — MaterialDesign DialogHost, 2-панельный layout (список чатов + сообщения), пузырьки, вложения, emoji-picker, диалог создания группы; 10) **ShellViewModel/ShellView** — кнопка «Чаты» в сайдбаре (корпоративный режим), NavChats style, DataTemplate; 11) **App.xaml.cs** — ChatsViewModel зарегистрирован; 12) **EF миграция AddGroupChatAndFileAttachment** применена к БД; 13) **Контакты UX** — тёмная тема редактирования (белый текст), совместные проекты в контекстном меню, OpenChatAsync с email-fallback, запросы в друзья (только send→accept), бейдж с кол-вом входящих запросов на кнопке «Контакты».
- **2026-05-14 (сессия 3)** — 4 исправления и пересборка: 1) **Календарь — только задачи из календаря + заметки** — `CalendarViewModel` переработан: показываются только задачи с явным `DueDate` (созданные через «Создать задачу» в панели дня) + заметки из раздела заметок; задачи из Kanban-проектов убраны полностью; нет fallback на «сегодня»; 2) **Иконки в ячейках** — `CalendarDayTask.IsNote` добавлен, в `CalendarView.xaml` заметки отображаются иконкой NoteOutline (синяя #1E88E5), задачи — цветной точкой приоритета; 3) **Server.exe не закрывается** — убрана проверка окружения для JWT Secret (теперь всегда fallback-ключ с предупреждением), `start-server.bat` + `start-client.bat` в `publish/`; 4) **Republish** — Server и Client пересобраны self-contained single-file exe в `publish/Server/` и `publish/Client/`.
- **2026-05-14 (сессия 2)** — 5 исправлений: 1) **Миграция AddChatLastRead** — создан недостающий `.Designer.cs` с атрибутом `[Migration]`, таблица `chat_last_reads` применена к БД; 2) **Шифрование сообщений чата** — `ChatEncryptionService` (ГОСТ Р 34.12-2015 Кузнечик), новые поля `text_cipher/text_nonce/text_hmac` в `chat_messages`, миграция `AddChatMessageEncryption` применена; все новые сообщения сохраняются зашифрованными, `MapDto` расшифровывает прозрачно; 3) **Реал-тайм в проектных чатах** — `SharedChatPanelViewModel.OpenProject()` теперь вызывает `await _hub.SubscribeAsync(projectId)`, клиент входит в SignalR-группу проекта; 4) **Задачи в календаре** — убраны задачи из совместных проектов (только личные Kanban-задачи), визуал ячеек переработан: маленькая цветная точка + текст задачи (вместо цветных баров); `CalendarViewModel` убран `ISharedProjectsApiService`; 5) **Выравнивание списка чатов** — `FlatIconButton` в `Generic.xaml` использует `{TemplateBinding HorizontalContentAlignment}` вместо хардкода `Center`, элементы списка чатов теперь прижаты к левому краю и не "прыгают"; **Emoji в SharedChatPanelView** — заменён нерабочий Popup на ContextMenu + code-behind (как в ChatsView).
- **2026-05-14 (сессия 5)** — Иконки приложений + republish: 1) **ConSecOrg.png → app.ico** — PowerShell конвертер PNG→ICO (6 размеров: 256/128/64/48/32/16px, PNG-формат внутри ICO); иконка добавлена в `src/ConSecOrg.Client/Assets/Icons/app.ico`; `<ApplicationIcon>` + `<Resource>` в `.csproj`; `Icon="Assets/Icons/app.ico"` в `MainWindow.xaml`; `IconSource="/Assets/Icons/app.ico"` на `tb:TaskbarIcon` (иконка в трее); сборочные мета-данные (AssemblyTitle/Company/Version); 2) **ConSecOrgServer.png → server.ico** — аналогично в `src/ConSecOrg.Server/Resources/server.ico`, `<ApplicationIcon>` в `.csproj`; 3) **Republish** — оба exe пересобраны self-contained single-file Release: `publish/Client/ConSecOrg.Client.exe` (272 МБ), `publish/Server/ConSecOrg.Server.exe` (123 МБ).
- **2026-05-14 (сессия 4)** — 4 улучшения чата: 1) **Исправлен CS8826** — параметр `OnIsOwnerChanged(bool v)` → `OnIsOwnerChanged(bool value)`, 0 предупреждений; 2) **Inline image preview** — `IsNonImageAttachment` в `ChatMsgVm`, `AttachmentUrl` маппится в `MapVm` через `ServerUrl/api/v1/files/{fileId}`; в `ChatsView.xaml` изображения отображаются прямо в пузырьках (MaxWidth=320, кликабельный Image), не-изображения — как раньше (кнопка-загрузка); 3) **Удаление своих сообщений** — `DELETE /api/v1/chats/messages/{id}` в `ChatController` (только отправитель, затем SignalR `MessageDeleted` broadcast); `DeleteChatMessageAsync` в `IChatApiService`/`ApiClient`; `DeleteMessageCommand` + `RemoveMessageLocally` в `ChatsViewModel`; `MessageDeleted` event в `SharedProjectsHubClient` → `OnMessageDeleted` обработчик; hover-кнопка мусорки (красная) рядом с пузырьком; 4) **Контекстное меню сообщений** — правый клик на пузырьке: свои = «Копировать текст» + «Удалить»; чужие = «Копировать текст»; биндинг через `Tag=ChatsViewModel` + `PlacementTarget.Tag.Command` паттерн.
